...
export default class MessengerBuilder extends BasicHandlerBuilder {
  onMessage(condition: Condition, handler: Handler) {
    this.on({
      condition: context => context.event.isMessage && condition(context),
      handler,
    });
    return this;
  }
  ...
  onText(pattern: Pattern, handler: Handler) {
    this.onMessage({
      condition: context => matchPattern(pattern, context.event.message.text),
      handler,
    });
    return this;
  }
  ...
  onPostback(condition: Condition, handler: Handler) {
    this.on({
      condition: context => context.event.isPostback && condition(context),
      handler,
    });
    return this;
  }
  ...
  onGetStarted(handler: Handler) {
    this.onPayload({
      pattern: constants.payload.GET_STARTED,
      handler,
    });
    return this;
  }
  ...
  onQuickReply(condition: Condition, handler: Handler) {
    this.onMessage({
      condition: context =>
        !!context.event.message.quick_reply && condition(context),
      handler,
    });
    return this;
  }
  ...
  onEcho(condition: Condition, handler: Handler) {
    this.onMessage({
      condition: context => context.event.message.is_echo && condition(context),
      handler,
    });
    return this;
  }
